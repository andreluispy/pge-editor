...
function modalConfirm() {
    ...
    if (objType == "rect"){
        toReturn = {
            name: 'NewRect' + countObjs('rect'),
            rendering: new Rect(mousePos.x.toFixed(2), mousePos.y.toFixed(2), 32, 32, "#0000ff")
        }
    }
    else if (objType == "circle"){
        toReturn = {
            name: 'NewCircle' + countObjs('circle'),
            rendering: new Circle(mousePos.x.toFixed(2), mousePos.y.toFixed(2), 32, "#0000ff")
        }
    }
    else if (objType == "text"){
        toReturn = {
            name: 'NewText' + countObjs('text'),
            rendering: new CText("Text", mousePos.x.toFixed(2), mousePos.y.toFixed(2), "#ffffff", "25px Arial")
        }
    }
...
function countObjs(objType){
    let i = 0
    for (obj in objs){
        if (objs[obj].rendering.type == objType){
            i++;
        }
    }
    return i
